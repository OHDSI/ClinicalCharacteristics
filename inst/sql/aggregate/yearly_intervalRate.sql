 ...
 SELECT
      ...
      FROM (
        SELECT
            ...
            g.statistic_type,
            (DATEDIFF(day, DATEADD(day, time_a, cohort_start_date), LEAST(cohort_end_date, DATEADD(day, time_b, cohort_start_date)))*1.0 + 1) / 365 AS interval_time
        FROM (
            SELECT
      ...
      LEFT JOIN (
        SELECT * FROM @pat_ts_tab WHERE statistic_type = 'yearly_intervalRate' AND patient_line = 'observedCount'
    ) r
